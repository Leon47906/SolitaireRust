...
async function loadImages() {
    ...
    for (const suit of suits) {
        for (let value = 1; value <= 13; value++) {
            ...
            promises.push(loadImage(`./Assets/Sprites/${key}.png`).then(img => { cardImages[key] = img; }));
        }
    }
    promises.push(loadImage('./Assets/Sprites/CardBackRed.png').then(img => { cardImages['back'] = img; }));
    promises.push(loadImage('./Assets/Sprites/GreenReload.png').then(img => { cardImages['recycle'] = img; }));
...
function drawState(state) {
    ...
    const dynamicOffset = maxPileSize > 1
        ? Math.min(TABLEAU_CARD_OFFSET, Math.floor(availableHeight / (maxPileSize - 1)))
        : TABLEAU_CARD_OFFSET;
    ...
    if (state.waste.length > 0) {
        ...
        drawCard(ctx, state.waste[state.waste.length - 1], MARGIN + PILE_SPACING, TOP_ROW_Y, isSelected);
    }

    state.foundation.forEach((pile,i) => {
        if (pile.length > 0)
            drawCard(ctx, pile[pile.length - 1], MARGIN + (3 + i) * PILE_SPACING, TOP_ROW_Y);

        else {
            ctx.save();
            ctx.strokeStyle = 'rgba(255,255,255,0.3)';
            ctx.lineWidth = 1;
            ctx.strokeRect(MARGIN + (3 + i) * PILE_SPACING + 0.5, TOP_ROW_Y + 0.5, CARD_W - 1, CARD_H - 1);
            ctx.restore();
        }
    });
    ...
    state.tableau.forEach((pile, col) => {
        ...
        pile.forEach((card, row) => {
            const isSelected = selected?.kind === 'tableau' && selected.col === col && row >= selected.row;
            drawCard(ctx, card, MARGIN + col * PILE_SPACING, TABLEAU_Y + row * dynamicOffset, isSelected);
        });
    });
    drawRestartButton();
